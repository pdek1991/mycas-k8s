...
client = Elasticsearch(
  ...
  api_key="MTY5TzFvNEJFT0JKOWJHamdwMXI6aHpSUUlQWE5SV1NWRjloS1FsTUVjQQ=="
)


log_pattern = re.compile(r'(?P<timestamp>\d{2}-\d{2}-\d{4}:\d{2}:\d{2}:\d{2}\.\d{3}) (?P<level>\w+) +(?P<message>.+)')

# Function to parse log lines and extract fields
def parse_log_line(log_line):
    match = log_pattern.match(log_line)
    if match:
        return {
            "timestamp": datetime.strptime(match.group("timestamp"), "%d-%m-%Y:%H:%M:%S.%f").isoformat(),
...
def send_logs_to_elasticsearch(file_pattern):
    ...
    for log_file in log_files:
        with open(log_file, 'r') as file:
            for line in file:
                ...
                if parsed_log:
                    documents.append({
                        "_index": "filebeat-logs",
                        "_source": parsed_log
                    })
    # Send documents to Elasticsearch
    response = client.bulk(body=documents, pipeline="ent-search-generic-ingestion")
    if response["errors"]:
...
connection_pool = mysql.connector.pooling.MySQLConnectionPool(pool_name="my_pool", pool_size=15, **db_config)
...
if __name__ == '__main__':
    #app.run()
    send_logs_to_elasticsearch("./Logs/mycas*")
